...
//Remove _amazon from Path 
$path = str_replace('/_amazon', '', $path);
...
if($account) {
    ...
    $curl = new \Curl\Curl('https://h10-re-frontend.s3.amazonaws.com'.$path);
    ...
    if($method == 'GET') {
        ...
        if(isset($responseHeaders['location'])) {
            header('Location: '.str_replace(['https://h10-re-frontend.s3.amazonaws.com'], [$cdnDomain], $responseHeaders['location']));
            die();
        }
        ...
        $responseHeaders->offsetUnset('x-amz-id-2');
        $responseHeaders->offsetUnset('x-amz-request-id');
